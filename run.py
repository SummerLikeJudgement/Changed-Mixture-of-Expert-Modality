...
torch.cuda.set_device(0) # 设置第0号GPU
...
def EMOE_run(
    ...
    args['feature_GSR'] = feature_GSR
    args['feature_ECG'] = feature_ECG
    if config:
        args.update(config)
    print(args)
...
def _run(args, num_workers=4, from_sena=False):
    ...
    model = getattr(emoe, 'EMOE')(args).cuda() # 创建并初始化EMOE，转移到GPU

    trainer = ATIO().getTrain(args)

    # 测试
    if args.mode == 'test':
        model.load_state_dict(torch.load('pt/biovid-unaligned.pth')) # 加载预训练模型
